...
大多数现有的姿态观察器/滤波器都依赖于小加速度假设(即，$v<<g$)，因此重力方向测量可以近似于上一小节中讨论的加速度计测量。然而，对于许多飞行器都有高速运动的场景，会产生比较大的线性加速度，姿态估计器的输出可能会有较大的误差。对于较大的线性加速度，可以将GPS线速度的补充测量与加速度计的测量相结合来估计飞行器的加速度，从而提高姿态估计的精度。
