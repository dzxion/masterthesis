...
%    fullwidth-stop  = {mapping},
  ...
  info = {
    title           = {多旋翼飞行器状态估计与控制算法研究},
    title*          = {Research on state estimation and control algorithm for multi-rotor aircraft},
    date            = {2022/5/25},
    author          = {邓正雄},
...
\begin{abstract}
  多旋翼飞行器具有体积小、悬停能力强、机动性强等优点，是在复杂环境中进行监视和搜救的理想平台。
  在这些应用场景中，飞行器必须能够自主飞行，以尽量减少操作员的工作量。
  随着近年来机器人技术的快速发展，能够实现自主飞行的飞行器已经广泛应用于各种场景当中。
  
  然而，要让飞行器在各种场景中安全地作业还存在许多需要解决的工程问题。
  其一，所有依赖于单一传感器的方法只能在特定的环境中发挥作用。
  因此，飞行器需要融合来自多个传感器的信息以提高状态估计的精度和鲁棒性。
  然而，目前的融合框架不支持以模块化和可扩展的方式融合多个异构传感器的信息。
  其二，每个传感器在运行过程中都有可能受到干扰。
  因此，飞行器需要对传感器的扰动进行处理，以保证飞行的安全性。
  然而，目前的传感器异常处理算法还不完善。
  其三，飞行器上用的是低成本MEMS传感器，这种传感器数据精度较差。
...
\gdutsubsection{状态估计}{State estimation}
...
\gdutsection{基于多频率和多延时测量的状态估计方法}{State estimation method based on multi-frequency and multi-delay measurement}
...
首先给出系统在高机动环境下的运行结果。
%通过比对本文估计器、板载飞控以及加速度计输出的姿态来评估估计器的鲁棒性。
以下是测试结果。
\begin{figure}[H]
	\centering
...
\gdutchapter{多旋翼飞行器控制算法设计}{Design of control algorithm for multi-rotor aircraft}
